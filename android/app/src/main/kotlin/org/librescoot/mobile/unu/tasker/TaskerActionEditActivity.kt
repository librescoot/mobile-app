...
import org.librescoot.mobile.unu.BatteryOptimization
import org.librescoot.mobile.unu.R

/**
 * The screen Tasker opens when the action is added or edited. Picking commits
 * immediately — there is exactly one setting, so a save button would only be in
 * the way. Backing out leaves the action as it was.
 */
class TaskerActionEditActivity : Activity() {
    ...
    private fun save(action: TaskerAction) {
        ...
        val result = Intent().apply {
            ...
            putExtra(TaskerPluginProtocol.EXTRA_REQUESTED_TIMEOUT, TaskerActionRunner.DEFAULT_TIMEOUT_MS.toInt())
