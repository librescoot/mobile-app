...
class TaskerActionReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (!TaskerAuthorization.accepts(context, settings)) {
            Log.w(TAG, "Rejected an action without valid authorization")
            reportFailureInline()
            return
        }
    ...
    /**
     * Tasker sends the completion intent URI-encoded as a String rather than as a
     * parcelled Intent. Other hosts may parcel it, so take either.
     */
