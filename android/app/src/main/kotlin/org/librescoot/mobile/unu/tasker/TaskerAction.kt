 ...
 * and waits for the scooter to confirm. The answer comes back through the
 * SharedPreferences file Flutter already uses to pass widget actions between
 * its isolates — same process, so the write is visible here at once.
 */
object TaskerActionRunner {
    ...
    /** A backstop for the Dart side never answering at all, not a judgement on how
     *  long the scooter may take: it sits above a cold start plus a 30s BLE
     *  connect timeout plus confirmation. */
    ...
    /** Must match `taskerResultServiceBlocked` in tasker_bridge.dart. */
    const val RESULT_SERVICE_BLOCKED = "service_blocked"

    /** Triggers [action] and blocks until it finishes, [timeoutMs] elapses, or the
     *  thread is interrupted. Never call this on the main thread. */
    ...
    private fun awaitResult(
        ...
        try {
            ...
            while (true) {
                ...
                // The listener wakes this the moment the result lands and the poll is
                // the backstop. Once the latch has fired await() stops blocking, so
                // sleep rather than spin out the deadline.
