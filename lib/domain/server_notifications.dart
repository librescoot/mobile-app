...
///                bundle id (`org.librescoot.mobile.unu`, plus the `.debug` and
///                `.profile` suffixes on Android). A pattern ending in `*` matches every
///                application id with that prefix, so one entry covers all build types.
///                Takes precedence over `branch`, which only exists for feeds published
///                before app ids.
